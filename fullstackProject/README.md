...
```bash
# фронт: vitest (144 теста) — форматтеры, зал, демо-движок (включая отзывы),
# сторы pinia (auth-сессия, «мои билеты», тема, lifecycle оплаты, отзывы),
# компоненты, экран оплаты /pay/:id, модалка отзывов и звёзды на карточках
cd apps/web && npm test

# API: юнит (92 теста) — логика брони, места/конфликт, pay/expire/cancel,
...
- На фронте: `/login` (вход и регистрация), токен в localStorage,
  ...
  с расписанием сеансов. В демо-режиме на Pages авторизации нет —
...
(баннер «демо-режим» в интерфейсе). Витрина — hero с чипами стека,
...
навигация). Собранная страница живёт
