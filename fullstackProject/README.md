...
```bash
# фронт: vitest (188 тестов) — форматтеры, зал, демо-движок (включая отзывы
...
# модалка отзывов, дашборд /admin/stats и SVG-график выручки, транспорт
# client.ts с refresh-перехватом 401 (single-flight) и страницы аккаунта.
...
- Страницы: `/profile` (две формы, чип юзера в шапке), `/forgot-password`,
  ...
  авторизации нет — страницы показывают пояснение.
