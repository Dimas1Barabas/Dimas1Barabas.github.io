...
```bash
# фронт: vitest (226 тестов) — форматтеры, зал, демо-движок (включая отзывы,
# сид-брони аналитики, симуляцию аккаунта и промокоды), сторы pinia
# (auth-сессия, «мои билеты», тема, lifecycle оплаты, отзывы, аналитика,
# промокоды), компоненты, экран оплаты /pay/:id с промо-блоком, модалка
...
# API: юнит (157 тестов) — логика брони, места/конфликт, pay/expire/cancel,
...
# откат транзакции), админ-аналитика (агрегаты, zero-fill дней, кэш),
# токены сессий (ротация, reuse-detection, гонки) и forgot/reset пароля
cd apps/api && npm test

# API: интеграционные (112) — полный HTTP-стек Nest (роутинг, ValidationPipe,
...
# исчерпания с честным ROLLBACK-фейком транзакции),
...
# со скидочной суммой, гонка исчерпания лимита;
...
открыта в демо всем — код можно создать свой.
