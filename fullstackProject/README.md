...
```bash
# фронт: vitest (84 теста) — форматтеры, зал, демо-движок, сторы pinia
# (включая auth-сессию, «мои билеты» и тему), компоненты
cd apps/web && npm test

# API: юнит (71 тест) — логика брони, места/конфликт, отмена/возврат,
# SSE, кэш, расписание сеансов, health, пользователи/посев админа,
# JWT-логин, retry/parking
cd apps/api && npm test

# API: интеграционные (48) — полный HTTP-стек Nest (роутинг, ValidationPipe,
# контроллеры → сервисы → фейковые Postgres/RabbitMQ/Redis на Map),
# включая 409-конфликт мест, изоляцию мест между сеансами, сагу отмены,
# SSE-стрим по живому HTTP, регистрацию/логин и guard'ы (401/403/роли)
...
3. `POST /api/bookings {sessionId, customerName, seats: ["5-7", "5-8"]}` —
   ...
   (фильм выводится из сеанса). Арбитр в гонке за место — составной
   уникальный констрейнт `(session_id, seat)` таблицы `seat_occupancy`:
   одно и то же место в разных сеансах одного фильма — независимые
   строки; проигравший в гонке получает 409 со списком занятых мест
   (`seatsTaken`).
...
- На фронте: `/login` (вход и регистрация), токен в localStorage,
  ...
  админу — `/admin`, форма нового фильма с расписанием сеансов. В демо-режиме
  на Pages авторизации нет — симуляция остаётся без токенов.
...
«демо-режим» в интерфейсе). Витрина — hero с чипами стека, скелетоны
загрузки, жанр-фильтры, карта зала с баром занятости, тёмная/светлая
тема (тумблер в шапке, без мигания при загрузке) и мобильная доводка
(скролл тела модалки, листаемая навигация). Собранная страница живёт
