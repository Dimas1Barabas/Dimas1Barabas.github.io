...
```bash
# фронт: vitest (106 тестов) — форматтеры, зал, демо-движок, сторы pinia
# (включая auth-сессию, «мои билеты», тему и lifecycle оплаты), компоненты,
# экран оплаты /pay/:id
cd apps/web && npm test

# API: юнит (80 тестов) — логика брони, места/конфликт, pay/expire/cancel,
# SSE, кэш, расписание сеансов, health, пользователи/посев админа,
# JWT-логин, retry/parking
cd apps/api && npm test

# API: интеграционные (56) — полный HTTP-стек Nest (роутинг, ValidationPipe,
...
1. `GET /api/movies` — SPA получает фильмы с расписанием: у каждого
   ...
   Redis-кэш (`movies:all:v2`, TTL 60 c) список не ходит в Postgres —
