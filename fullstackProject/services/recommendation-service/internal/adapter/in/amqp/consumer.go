// Package amqp — входящий адаптер: слушает сигналы зрителей на обмене
...
// NestJS API в момент, когда под рукой вся связка «зритель × фильм»:
// подтверждённая бронь (handleProcessed) и созданный отзыв.
const (
	keyBookingConfirmed = "recommendation.booking.confirmed" // NestJS API: CONFIRMED
	keyReviewCreated    = "recommendation.review.created"    // NestJS API: отзыв записан
)

const (
	retryHeader = "x-retry-count"
	queueName   = "recommendation.signals"
)

// Consumer связывает очередь сигналов со сценарием рекомендаций.
...
// в main, как у notification-service.
func (c *Consumer) Run(ctx context.Context) error {
	...
	// по одному сообщению за раз: профиль строится из всей истории —
	// порядок сигналов на скор не влияет, спешить некуда
	...
	log.Printf("recommendation слушает %s / %s + %s", queueName, keyBookingConfirmed, keyReviewCreated)
...
// handle: сообщение → доменный сигнал → сценарий. Ядовитые события
...
// зеркально топологии notification-service. Retry-очередь нужна на каждый
...
func (c *Consumer) declareTopology(ch *amqp091.Channel) error {
	...
	for _, key := range []string{keyBookingConfirmed, keyReviewCreated} {
